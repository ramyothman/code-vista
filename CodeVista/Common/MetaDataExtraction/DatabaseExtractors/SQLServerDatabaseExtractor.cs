...
namespace Common.MetaDataExtraction.DatabaseExtractors
{
    public class SQLServerDatabaseExtractor : DatabaseExtractor
    {
        ...
        public override void GetColumns(Common.Entities.MetaDataSchema.Table table)
        {
            ...
            string query = String.Format("Select c.CONSTRAINT_NAME,c.Constraint_Type,t.Column_Name,c.Table_Name,c.Table_Schema From {0}.INFORMATION_SCHEMA.TABLE_CONSTRAINTS as c Inner join {0}.INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE as t on c.Constraint_Name = t.Constraint_Name", table.ParentDatabase.Name);
        ...
        public override void GetColumns(Common.Entities.MetaDataSchema.Database database)
        {
            ...
            string query = "Select c.CONSTRAINT_NAME,c.Constraint_Type,t.Column_Name,c.Table_Name,c.Table_Schema From " + database.Name + ".INFORMATION_SCHEMA.TABLE_CONSTRAINTS as c Inner join " + database.Name + ".INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE as t on c.Constraint_Name = t.Constraint_Name";
            //sqlObjectsTable.Load(table.ParentDatabase.ParentProject.ExtractorManager.SelectStatement("Select * From " + table.ParentDatabase.Name + ".sys.objects"), LoadOption.OverwriteChanges);
